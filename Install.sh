...
install_node_modules() {
    local effective_options="${NODE_OPTIONS:---max-old-space-size=${LIGHT_NODE_HEAP_MB}}" status
    info "正在以低内存、单任务模式安装运行依赖..."
    (
        cd "$ST_DIR" || exit 1
        export NODE_ENV=production
        export npm_config_jobs="${npm_config_jobs:-1}"
        if command -v nice >/dev/null 2>&1; then
            NODE_OPTIONS="$effective_options" nice -n "$LIGHT_NICE_LEVEL" \
                npm install --no-save --no-audit --no-fund --no-progress \
                    --loglevel=error --omit=dev
        else
            NODE_OPTIONS="$effective_options" \
                npm install --no-save --no-audit --no-fund --no-progress \
                    --loglevel=error --omit=dev
        fi
    )
...
start_sillytavern() {
    local effective_options="${NODE_OPTIONS:---max-old-space-size=${LIGHT_NODE_HEAP_MB}}"
    if ! has_sillytavern; then
        warn "未检测到 SillyTavern，将自动安装固定版 ${ST_VERSION}。"
        install_fixed_version || return
    fi
    ...
    info "正在以轻量模式启动：内存上限 ${LIGHT_NODE_HEAP_MB} MB，线程池 ${LIGHT_UV_THREADS}。"
    if command -v termux-open-url >/dev/null 2>&1; then
        ( sleep 6; termux-open-url 'http://127.0.0.1:8000' >/dev/null 2>&1 || true ) &
    fi
    if command -v nice >/dev/null 2>&1; then
        (cd "$ST_DIR" && NODE_ENV=production NODE_OPTIONS="$effective_options" \
            UV_THREADPOOL_SIZE="$LIGHT_UV_THREADS" \
            nice -n "$LIGHT_NICE_LEVEL" node server.js)
    else
        (cd "$ST_DIR" && NODE_ENV=production NODE_OPTIONS="$effective_options" \
            UV_THREADPOOL_SIZE="$LIGHT_UV_THREADS" node server.js)
    fi
}
...
show_status() {
    ...
    printf "Git 提交：%s\n" "${commit:-无}"
    printf "Node.js：%s\n" "$(node -v 2>/dev/null || echo 未安装)"
    printf "轻量限制：%s MB / %s 线程\n" "$LIGHT_NODE_HEAP_MB" "$LIGHT_UV_THREADS"
    printf "管理器：v%s（构建 %s）\n" "$SCRIPT_VERSION" "$SCRIPT_BUILD"
}
...
main_menu() {
    while true; do
        clear 2>/dev/null || true
        printf "%b========================================\n" "$CYAN"
        printf "  SillyTavern %s Termux 轻量管理器\n" "$ST_VERSION"
        printf "  管理器 v%s\n" "$SCRIPT_VERSION"
        printf "========================================%b\n" "$RESET"
        printf "脚本更新：%s\n" "$UPDATE_STATUS"
        printf "酒馆状态：%s\n\n" "$(has_sillytavern && echo 已安装 || echo 未安装)"
